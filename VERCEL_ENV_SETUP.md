...
#### Переменная 5: `VITE_TELEGRAM_BOT_TOKEN` (опционально, для Telegram Mini App)

⚠️ **ВАЖНО: Не перепутайте местами!**

- **Ключ (Key):** `VITE_TELEGRAM_BOT_TOKEN` ← Имя переменной
- **Значение (Value):** `8346758333:AAFJgFLdLHe4wSRWjZReGFYvwNsOOWYScyY` ← Токен вашего Telegram бота
- **Environment:** Выберите все (Production, Preview, Development)

**НЕ ПУТАЙТЕ:**
- ❌ Ключ = токен бота (это вызовет ошибку!)
- ✅ Ключ = `VITE_TELEGRAM_BOT_TOKEN`, Значение = токен

⚠️ **БЕЗОПАСНОСТЬ:** Не публикуйте токен бота в публичных репозиториях!
